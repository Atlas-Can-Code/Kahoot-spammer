...
function ads() {
	console.clear();
	console.log('--------------------------');
	console.log('Kahoot Bots 2.1');
	console.log('Made by spencer');
	console.log('---------------------------------------\n');
}
ads();
...
antibotmode = readline.question(
	'Use the new BETA antibot mode? [turns on random name and name bypass] [works most of the time] (y/n)> '
);
if (antibotmode == 'y') {
...
bots = readline.question(
	'Enter number of bots users [Do not exceed 480 bots or it will crash] = '
);
if (antibotmode == 'y') {
	ranname = true;
	er = 'n';
	...
	if (ranname == 'y') {
		...
		ranname = false;
		botname = readline.question('Enter desired bot name = ');
		botprefix = '';
	}
	er = readline.question(
		'Use name bypass? [pretty sure it bypasses character limit] (y/n)> '
	);
}
...
if (usercontrolled == 'y') {
	usercontrolled = true;
	beepis = readline.question(
		'Make Beep sound when the bots need controlling? (y/n)> '
	);
} else {
...
function spam() {
	if (repeattimes == bots) {
		console.log('All bots have connected to the kahoot server');
	} else {
		repeattimes++;
		...
		if (question.type == 'jumble') {
			console.log(
				'User controlling is not currently available for jumbles. The bot ' +
					name +
					' responded with a random generated answer.'
			);
			setTimeout(() => {
				client.answer(
					getRandomInt(
